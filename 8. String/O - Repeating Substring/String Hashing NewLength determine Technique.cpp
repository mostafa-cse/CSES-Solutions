...
struct HashString {
    const int M = 1111111111111111111;  // M is a prime.
    const int C = uniform_int_distribution<int>(0.1 * M, 0.9 * M)(rng);

    int mul(int a, int b) { return __int128(a) * b % M; }
    ...
    int hash(int l, int r) {
        int h = sums[r] - mul(sums[l], pows[r - l]);
        return (h % M + M) % M;
    }
};
...
int32_t main() {
    ...
    function<int(int)> find = [&](int len) -> int {
        set<int> hashes;
        for (int i = 0; i <= n - len; i++) {
            int h = hs.hash(i, i + len);
